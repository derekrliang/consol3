...
function handleNoGeolocation(errorFlag) {
    ...
    maps[map_index].setCenter(options.position);
}
...
$(document).ready(function() {
    ...
    function loadMapScript() {
        ...
        //script.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyAlyUN2TTsAuiRXQmNdQXm3-JVzgzmjm0c?v=3.exp&' + 'callback=map_initialize';
        script.src = 'https://maps.googleapis.com/maps/api/js?v=3.exp&' + 'callback=map_initialize';
        document.body.appendChild(script);
    }

    commands["map"] = function(args) {
        if (map_make_ready) {
            map_make_ready = false;
            loadMapScript();
            printHtml("<div class=\"map-canvas\" id=\"map-canvas-" + map_index + "\"></div>");
        }
    };
